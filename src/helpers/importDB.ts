...
export async function importDB(db: PouchDB.Database, data: { regular: any[]; local: any[] }) {
  try {
      ...
      try {
        ...
        if (err.name === 'conflict') {
          try {
            ...
            const updateResult = await db.put({ ...doc, _rev: existingDoc._rev });
          } catch (innerErr: any) {
          ...
          throw new Error(`Failed to import document ${doc._id}: ${err.message}`);
        }
      }
    }
  } catch (error) {
